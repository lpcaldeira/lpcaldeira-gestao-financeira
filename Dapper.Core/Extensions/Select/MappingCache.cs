                ...
                var destination = destinationExpressions
                    .Select(parameter => new
                    {
                        ...
                        Property = parameter.Type
                            ...
                            .FirstOrDefault(property => property.PropertyType == sourceExpression.Type 
                                || IsSubclassOf(sourceExpression.Type, property.PropertyType) 
                                || property.PropertyType.IsAssignableFrom(sourceExpression.Type))
                    }).FirstOrDefault(parameter => parameter.Property != null);

                if (destination == null)
                {
                    throw new InvalidOperationException(string.Format("No writable property of type {0} found in types {1}.", 
                        sourceExpression.Type.FullName, string.Join(", ", destinationExpressions.Select(parameter => parameter.Type.FullName))));
                }
